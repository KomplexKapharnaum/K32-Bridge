...
class Midi2MQTT(Midi2Base):
    def __init__(self, broker):
        ...
        print(f"-- ELP: sending to broker at {broker}\n")
    ...
    def push(self):
        for i in range(16):
            if self.dirty[i] > 0:
                ...
                payloadFormated = ""
                for k in range(FIXTURE_SIZE):
                    if k > 0: payloadFormated = payloadFormated+"|"
                    payloadFormated = payloadFormated+str(self.payload[i][k])
                self.mqttc.publish('k32/'+dev+'/leds/frame', payload=payloadFormated, qos=1, retain=False)
                print('k32/'+dev+'/leds/frame', payloadFormated)
